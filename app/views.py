...
from .forms import LoginForm, EditForm, PostForm, SearchForm, EditPostForm
...
def edit_post(id):
	...
	edit_form = EditPostForm(obj=post)
	if post is None:
		flash(gettext('Post not found'))
		return redirect(url_for('index'))
	if post.author.id != g.user.id:
	...
	if request.method == 'POST' and edit_form.validate_on_submit:
		...
		db.session.add(post)
		db.session.commit() 
		flash(gettext('Your post has been edited.'))
		return redirect(url_for('index'))
	else: 
		edit_form.post.data = post.body
	return render_template('editpost.html', id=post.id, form=edit_form)
